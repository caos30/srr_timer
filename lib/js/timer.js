...
function js_json_get_projects(first_load){
	...
	var url_json = 'lib/json/json.php?op=get_times_today&lang='+settings['lang'];
        if (first_load=='1') url_json += '&first_load=1';
        //alert(url_json);
	$.getJSON(
		url_json,
		function(data){ 
			...
			$.each(data.a_projects, function(k, val) {
				...
				a_chronos[i]['n_times'] = parseInt(a_chronos[i]['n_times']);
                                /* == sum today times == */
                                a_chronos[i]['all'] = a_chronos[i]['all'] + a_chronos[i]['today'];
                                if (a_chronos[i]['today'] > 0){
                                    a_chronos[i]['n_times'] = a_chronos[i]['n_times'] + 1;
                                }
			});
...
/* ============ MERGE process ============ */
    ...
    function js_merge_button(id_project){
            ...
            var id_project_db = a_chronos[id_project]['_id_'];
            for (i2 in a_chronos){
                    if (a_chronos[i2]['_id_'] != id_project_db){
    ...
    function js_save_div_merge(idp_from,idp_to){
            var id_project_from = $('#div_merge').attr('rel');
                id_project_from = a_chronos[id_project_from]['_id_'];
            var id_project_to = $('#f_project_to').val();
            $('#div_debug img').fadeIn('slow');
            if (itask>0){
                    it = itask;
                    js_stop(itask);
            }
            var url_json = 'lib/json/json.php?op=merge_project&id_project_from=' + id_project_from + '&id_project_to=' + id_project_to;
...
/* ============ ADD NEW TASK process ============ */
    ...
    function js_save_div_add(){
            var url_json = 'lib/json/json.php?op=add_project';
            $('#txt_debug').val(url_json);
            $.getJSON(
                    url_json,
                    {'title': $('#f_new_project_name').val()},
                    function(data){ 
                            $('#div_debug img').fadeOut('slow');
                            n_chronos++;
                            var inew = n_chronos;   
...
/* ============ STATS process ============ */
    ...
    function js_stats_button(id_project){
            ...
            if (id_project == '0'){
                $('#div_stats_task_name').hide();
                var url_json = 'lib/json/json.php?op=get_times_project&id=0';
            }else{
                $('#div_stats_task_name').show();
            ...
            $.getJSON(
                    ...
                    function(data){ 
                            ...
                            $.each(data.a_project.times, function(id, val) {
                                    ...
                                    var perc = parseInt((val.t/max)*90);
                                    var bar_width = parseInt(perc*1.5);
                                    html += "<tr><td class='n_w'><b>" + datef + "</b></td><td>" + js_sec2hms(parseInt(val.t)) + "</td><td><img src='lib/images/pixel-vacio.gif' class='bar' style='width:"+bar_width+"px;' title='"+perc+" %'></span></td></tr>";  
                            });
                            html += "</table>";
                            /*eval(data.script);*/
    ...
    function js_update_times(){
            ...
            for (i2 in a_chronos){
                    ...
                    if (i2==itask){
                            sec += chrono;
                            /*
                             * this updating of the title bar was wonderful,
                             * but i should disable it because some browsers WRITE ON HARD DISK this change and then this webapp was WRITTING ON DISK ONCE PER SECOND!!?? :O
                             * 
...
/* ============ misc. ============ */
    ...
    function js_save(callback){
            ...
            */
            // JSON 
            var url_json = 'lib/json/json.php?op=save_projects_data';
            $('#txt_debug').val(url_json);
            $.getJSON(
                    url_json,
                    {'data': data},
    ...
    function js_save_settings(){
            ...
            /* == send data to server == */
                ...
                /* data:           _id_ [:] 1 [|] username [:] sergi [|]  .... [|] show [:] active [|] lang [:] en       */
                    var url_json = 'lib/json/json.php?op=save_settings';
                    $('#txt_debug').val(url_json);
