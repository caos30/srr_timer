...
function js_json_get_projects(first_load){
	...
	$.getJSON(
		...
		function(datos){ 
			var html = "";
			var i=0;
			$.each(datos.a_projects, function(k, val) {
				i++;	
			...
			$('#current_date').html(datos.current_date);
                        //eval(datos.script);
                        var it_temp = itask;
...
function js_render_projects_table(){
	...
	for (i2 in a_chronos){
				...
				+ "<a href='#' rel='stop' class='c_r2'><i class='fa fa-stop'></i></a>" 
    ...
    function js_save_div_add(){
            ...
            $.getJSON(
                    ...
                    function(datos){ 
                            ...
                            a_chronos[inew]['_id_'] = datos._id_;
...
/* ============ STATS process ============ */
    ...
    function js_stats_button(id_project){
            ...
            $.getJSON(
                    ...
                    function(datos){ 
                            ...
                            var max = datos.a_project.max_time_day;
                            $.each(datos.a_project.times, function(id, val) {
    ...
    function js_stats_update(interval){
            if (interval=='days'){
                    ...
                    $('#div_stats .bt[rel=days]').addClass('selected');
                    var a_datos = idatos.a_project.times; 
                    var max = idatos.a_project.max_time_day;
            }else if (interval=='months'){
                    $('#div_stats .bt').removeClass('selected');
                    $('#div_stats .bt[rel=months]').addClass('selected');
                    var a_datos = idatos.a_project.months;  
                    var max = idatos.a_project.max_time_month; 
            }else{
                    $('#div_stats .bt').removeClass('selected');
                    $('#div_stats .bt[rel=years]').addClass('selected');
                    var a_datos = idatos.a_project.years;  
                    var max = idatos.a_project.max_time_year; 
            }
            $('#div_stats_list').html("");
            var html = "<table style='width:100%;' cellpadding='0'>";
            $.each(a_datos, function(id, val) {
                    var datev = new String(val.d);
                    if (interval=='days')
                            var datef = datev.substr(6,2) + "-" + datev.substr(4,2) + "-" + datev.substr(0,4);
                    else if (interval=='months')
            ...
            */
            ...
            $.getJSON(
                    ...
                    function(datos){ 
                            //eval(datos.script);
                            $('#div_debug img').fadeOut('slow');
                            if (datos.updated_today_times=='1') js_json_get_projects();
                            $('#current_date').html(datos.current_date);
